test_that("pg_filter_by_af correctly subsets variants based on frequency", {
  ...
  res_common <- pg_filter_by_af(test_data, min_af = 0.1, max_af = 0.9)
  expect_equal(nrow(res_common), 1)
  expect_equal(res_common$variant_id, "v_seg")

  #  Test: Inclusive Range (0 to 1)
  # All should pass
  res_all <- pg_filter_by_af(test_data, min_af = 0, max_af = 1)
  expect_equal(nrow(res_all), 3)

  # Test: Fixed Alternate Only
  res_fixed <- pg_filter_by_af(test_data, min_af = 0.9, max_af = 1)
...
test_that("pg_filter_by_af handles empty inputs and no-passers", {
  ...
  expect_error(pg_filter_by_af(NULL), "genotype matrix is empty")
  ...
  expect_warning(
    pg_filter_by_af(test_data, min_af = 0.5, max_af = 1),
    "No variants passed"
  )
})