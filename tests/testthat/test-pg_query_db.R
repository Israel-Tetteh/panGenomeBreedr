test_that("pg_query_db retrieves and unpacks genomic data correctly", {
  ...
  dittodb::with_mock_path("fixtures", {
    dittodb::with_mock_db({
      ...
      res_var <- pg_query_db(con, "variants", chr, st, en)
      ...
      res_gt <- pg_query_db(con, "genotypes", chr, st, en)
...
test_that("pg_query_db handles empty results gracefully", {
  ...
  res_empty <- pg_query_db(con, "variants", "Chr99", 1, 100)
