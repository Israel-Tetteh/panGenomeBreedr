...
The package's bundled example database (the same `pangenome_scale_db` used throughout this vignette) includes a small PCIL data slice, so the walkthrough below runs entirely offline. Set `connect_db_mode = 'online'` instead to query the full public **Curated Sorghum Pangenome-Scale Variant Resource**.
...
pcil_data <- fetch_pcil_data(con = con_demo)
...
Of the six HIGH/MODERATE impact variants annotated for this gene, we settle on two tightly linked missense variants as our putative causal variants for KASP marker design:
